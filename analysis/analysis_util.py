...
#         # Set the joint state for the current node
#         env.C.setJointState(q)

#         # Extract world positions for the robot frames
#         positions = [env.C.getFrame(r).getPosition() for r in env.robots]
#         if not positions:
#             print(f"Warning: No positions extracted for idx={idx}.")
#             continue

#         # Add a marker for the current node
#         for r in range(len(env.robots)):
#             color = colors[mode_idx]
#             node_name = f"node_{pkl_file_idx}_{idx}_{r}"
#             env.C.addFrame(node_name) \
#                 .setParent(table if table else None) \
#                 .setShape(ry.ST.sphere, size=node_size) \
#                 .setColor(color) \
#                 .setPosition(positions[r])
      
# def visualize_nodes(env, pkl_folder):
  
#     table = env.C.getFrame("table") if "table" in env.C.getFrameNames() else None
#     #Load pkl files
#     pkl_files = sorted(
...
#         # Visualize the RRT
#         print(idx)
#         visualize_rrt_with_joint_states(idx, env, tree, table)
