...
class rai_two_dim_env_single(SequenceMixin, rai_env):
    def __init__(self, agents_can_rotate=True):
        self.C, keyframes = rai_config.make_2d_rai_env_sinlge(
            agents_can_rotate=agents_can_rotate
        )
        # self.C.view(True)
...
class rai_hallway_two_dim_single(SequenceMixin, rai_env):
    def __init__(self, agents_can_rotate=True):
        self.C, keyframes = rai_config.make_two_dim_tunnel_env_single(
            agents_can_rotate=agents_can_rotate
        )
        # self.C.view(True)
        ...
        self.tasks = [
            Task(["a1"], SingleGoal(keyframes[0])),
            Task(["a1"], SingleGoal(keyframes[1])),
        ]
