...
class BidirectionalRRTstar(BaseRRTstar):
    ...
    def update_tree(
        self, mode: Mode, n: Node, n_parent: Node, cost_to_parent: NDArray
    ) -> None:
        """
    ...
    def connect(self, mode: Mode, n_new: Node) -> None:
        ...
        if (
            self.config.birrtstar_version == 1
            or self.config.birrtstar_version == 2
            and self.trees[mode].connected
        ):  # Based on paper Bi-RRT* by B. Wang
            ...
            self.config.birrtstar_version == 2 and not self.trees[mode].connected
        ):  # Based on paper RRT-Connect by JJ. Kuffner/ RRT*-Connect by S.Klemm
        ...
        transition_node = self.get_transition_node(
            mode, self.transition_node_ids[mode][-1]
        )
    ...
    def extend(
        self, mode: Mode, n_nearest_b: Node, n_new: Node, dist
    ) -> Optional[Node]:
        """
        ...
        while True:
            ...
            if (
                not state_new or np.equal(state_new.q.state(), q.state()).all()
            ):  # Reached
                return n_nearest_b
    ...
    def save_tree_data(self) -> None:
        ...
        data["all_nodes"] = [
            ...
            for m in self.modes
            for id in self.trees[m].get_node_ids_subtree("A")
        ]
