...
from multi_robot_multi_goal_planning.planners.sampling_informed import (
    InformedSampling as BaseInformedSampling,
)
...
class Relatives:
    ...
    __slots__ = ["parent", "children", "cost_to_parent", "fam"]
...
class BaseTree:
    ...
    def add_vertex(self, node: BaseNode) -> None:
        ...
        self.vertices_batch_array = self.ensure_capacity(
            self.vertices_batch_array, position
        )
        ...
        if np.any(valid_mask):
            ...
            for idx in sorted_indices:
                ...
                if n.id in goal_node_ids:
                    continue
                if n in node.whitelist or self.is_edge_collision_free(
                    n.state.q, node.state.q, node.state.mode
                ):
                    c_min = c_new_tensor[idx]
                    c_min_to_parent = batch_cost[idx]
                    n_min = n
        ...
        if np.any(improvement_mask):
            ...
            for idx in improved_indices:
                ...
                if n_near.is_transition and n_near.is_reverse_transition:
                    continue
                if (
                    n_near == node.forward.parent
                    or n_near.cost == np.inf
                    or n_near.id == node.id
                ):
                    continue
                if node in n_near.forward.children:
                    pass
                if (
                    node.state.mode == n_near.state.mode
                    or node.state.mode == n_near.state.mode.prev_mode
                ):
                    if node in n_near.whitelist or self.is_edge_collision_free(
                        node.state.q, n_near.state.q, node.state.mode
                    ):
                        edge_cost = float(batch_cost[idx])
                        self.update_connectivity(
                            node, n_near, edge_cost, node.cost + edge_cost
                        )
...
class BaseGraph(ABC):
    ...
    def __init__(
        ...
        self.operation.update(
            self.root, lb_cost_to_go=np.inf, cost=0, lb_cost_to_come=0.0
        )
        self.batch_dist_fun = batch_dist_fun
        self.batch_cost_fun = batch_cost_fun
        self.is_edge_collision_free = is_edge_collision_free
        ...
        self.unit_n_ball_measure = ((np.pi**0.5) ** self.dim) / math.gamma(
            self.dim / 2 + 1
        )
        self.new_path = None
        self.weight = 1
    ...
    def add_node(
        self, new_node: BaseNode, cost: float = np.inf, lb_cost_to_go: float = np.inf
    ) -> None:
        """
    ...
    def add_transition_nodes(
        self, transitions: List[Tuple[Configuration, Mode, List[Mode]]]
    ) -> None:
        """
        ...
        for q, this_mode, next_modes in transitions:
            ...
            if this_mode in self.transition_node_ids:
                if (
                    self.transition_or_goal_is_already_present(node_this_mode)
                    is not None
                ):
                    continue
            if next_modes is not None:
                ...
                for next_mode in next_modes:
                    node_next_mode = self.node_cls(
                        self.operation, State(q, next_mode), True
                    )
    ...
    def add_path_states(
        ...
        for i in range(len(path)):
            is_transition = False
            next_mode = None
            if i < len(path) - 1 and path[i].mode != path[i + 1].mode:
                is_transition = True
            ...
            if is_transition:
                if (
                    parent.is_transition
                    and parent.is_reverse_transition
                    and self.reverse_transition_is_already_present(node)
                ):
                    continue
                ...
                if n is not None:
                    if parent.id not in n.forward.fam:
                        edge_cost = float(
                            self.batch_cost_fun([parent.state], [n.state])
                        )
                        self.update_connectivity(
                            parent, n, edge_cost, parent.cost + edge_cost
                        )
                        ...
                        if n.transition_neighbors and n not in self.virtual_goal_nodes:
                            ...
                            self.update_connectivity(
                                n, transition_neighbor, 0.0, n.cost
                            )
                            if rewire:
                                r = self.get_r_star(
                                    self.tree[next_mode].cnt,
                                    space_extent,
                                    self.unit_n_ball_measure,
                                )
                                batch_cost, n_near_costs, node_indices = self.tree[
                                    next_mode
                                ].neighbors(transition_neighbor, None, r)
                                self.tree[next_mode].rewire(
                                    transition_neighbor,
                                    batch_cost,
                                    n_near_costs,
                                    node_indices,
        ...
        if is_transition:
            ...
            if next_mode is not None:
                ...
                for all_next_mode in all_next_modes:
                    node_next_mode = self.node_cls(
                        self.operation,
                        State(node.state.q, all_next_mode),
                        is_transition,
                    )
                    ...
                    if all_next_mode in self.tree and rewire:
                        r = self.get_r_star(
                            self.tree[all_next_mode].cnt,
                            space_extent,
                            self.unit_n_ball_measure,
                        )
                        batch_cost, n_near_costs, node_indices = self.tree[
                            node_next_mode.state.mode
                        ].neighbors(node_next_mode, None, r)
                        self.tree[all_next_mode].rewire(
                            node_next_mode, batch_cost, n_near_costs, node_indices
                        )
                    self.add_vertex_to_tree(node_next_mode)
                    self.add_transition_node(
                        node_next_mode, reverse=True, cost=node.cost
                    )
            ...
            if node.state.mode in self.tree and rewire:
                r = self.get_r_star(
                    self.tree[node.state.mode].cnt,
                    space_extent,
                    self.unit_n_ball_measure,
                )
                batch_cost, n_near_costs, node_indices = self.tree[
                    node.state.mode
                ].neighbors(node, parent, r)
                # true_parent = parent
                # true_edge_cost = float(self.batch_cost_fun([parent.state], [node.state]))
                true_parent, true_edge_cost = self.tree[node.state.mode].find_parent(
                    node,
                    parent,
                    batch_cost,
                    n_near_costs,
                ...
                self.update_connectivity(
                    true_parent,
                    node,
                    float(true_edge_cost),
                    true_parent.cost + float(true_edge_cost),
                )
                self.tree[node.state.mode].rewire(
                    node, batch_cost, n_near_costs, node_indices
                )
            else:
                edge_cost = float(self.batch_cost_fun([parent.state], [node.state]))
                self.update_connectivity(
                    parent, node, edge_cost, parent.cost + edge_cost
                )

            self.add_node(node, cost=node.cost)
            self.add_vertex_to_tree(node)

    def transition_or_goal_is_already_present(
        self, node: BaseNode
    ) -> Optional[BaseNode]:
        """
        ...
        """
        if (
            node.state.mode in self.transition_node_ids
            and len(self.transition_node_ids[node.state.mode]) > 0
        ):
            configs_list = [
    ...
    def update_cache(self, key: Mode) -> None:
        ...
        if key in self.node_ids and key not in self.node_array_cache:
            ...
            self.node_array_cache[key] = np.empty(
                (len(ids), self.dim), dtype=np.float64
            )
            for i, id in enumerate(ids):
                self.node_array_cache[key][i] = self.nodes[id].state.q.q
        if (
            key in self.transition_node_ids
            and key not in self.transition_node_array_cache
        ):
            ids = self.transition_node_ids[key]
            self.transition_node_array_cache[key] = np.empty(
                (len(ids), self.dim), dtype=np.float64
            )
            for i, id in enumerate(ids):
                self.transition_node_array_cache[key][i] = self.nodes[id].state.q.q
        if (
            key in self.reverse_transition_node_ids
            and key not in self.reverse_transition_node_array_cache
        ):
            ids = self.reverse_transition_node_ids[key]
            self.reverse_transition_node_array_cache[key] = np.empty(
                (len(ids), self.dim), dtype=np.float64
            )
            for i, id in enumerate(ids):
                self.reverse_transition_node_array_cache[key][i] = self.nodes[
                    id
                ].state.q.q
        if (
            key.prev_mode is None
            and key not in self.reverse_transition_node_array_cache
        ):
    ...
    def get_neighbors(
        ...
        if (
            key in self.reverse_transition_node_array_cache
            and len(self.reverse_transition_node_ids[key]) > 0
        ):
            ...
            if (
                len(self.reverse_transition_node_array_cache[key]) == 1
                and self.reverse_transition_node_ids[key][0] not in node_ids
            ):
                r_star = 1e6
        ...
        if node.is_transition and node.transition_neighbors:
            all_ids = np.concatenate(
                (all_ids, [n.id for n in node.transition_neighbors])
            )
            arr = np.vstack(
                [arr, [n.state.q.state() for n in node.transition_neighbors]]
            )

        assert node.id in all_ids, " ohhh nooooooooooooooo"
        # remove node itself
        ...
        self.neighbors_batch_cost_cache[node.id] = self.batch_cost_fun(
            node.state.q, arr
        )
        self.blacklist_cache[node.id] = set()
        if self.including_effort:
            self.neighbors_batch_effort_cache[node.id] = (
                self.batch_dist_fun(node.state.q, arr, c="max")
                / self.collision_resolution
            )
        if first_search:
            self.tot_neighbors_batch_cost_cache[node.id] = (
                self.neighbors_batch_cost_cache[node.id]
            )
            self.tot_neighbors_id_cache[node.id] = all_ids
            if self.including_effort:
                self.tot_neighbors_batch_effort_cache[node.id] = (
                    self.neighbors_batch_effort_cache[node.id]
                )
            return all_ids
        return self.update_neighbors(node, True)

    def update_neighbors_with_family_of_node(
        self, node: BaseNode, update: bool = False
    ) -> NDArray:
        """
        ...
        if self.search_init_sol:
            if update:
                self.tot_neighbors_id_cache[node_id] = self.neighbors_node_ids_cache[
                    node_id
                ]
                self.tot_neighbors_batch_cost_cache[node_id] = (
                    self.neighbors_batch_cost_cache[node_id]
                )
                if self.including_effort:
                    self.tot_neighbors_batch_effort_cache[node_id] = (
                        self.neighbors_batch_effort_cache[node_id]
                    )
        ...
        assert not (current_fam & blacklist), "items from the set are in the array"
        ...
        if not update and not fam_changed:
            return self.tot_neighbors_id_cache.get(
                node_id, self.neighbors_node_ids_cache[node_id]
            )
        ...
        if new_ids.size > 0:
            new_arr = np.array(
                [self.nodes[id_].state.q.q for id_ in new_ids], dtype=np.float64
            )
            new_costs = self.batch_cost_fun(node.state.q, new_arr)
            self.tot_neighbors_id_cache[node_id] = np.concatenate((new_ids, base_ids))
            self.tot_neighbors_batch_cost_cache[node_id] = np.concatenate(
                (new_costs, base_cost)
            )
            if self.including_effort:
                new_effort = (
                    self.batch_dist_fun(node.state.q, new_arr, c="max")
                    / self.collision_resolution
                )
                self.tot_neighbors_batch_effort_cache[node_id] = np.concatenate(
                    (new_effort, self.neighbors_batch_effort_cache[node_id])
                )
        else:
    ...
    def update_neighbors(self, node: BaseNode, update: bool = False) -> NDArray:
        ...
        if blacklist_diff:
            ...
            if np.any(~mask):
                ...
                self.neighbors_batch_cost_cache[node.id] = (
                    self.neighbors_batch_cost_cache[node.id][mask]
                )
                if self.including_effort:
                    self.neighbors_batch_effort_cache[node.id] = (
                        self.neighbors_batch_effort_cache[node.id][mask]
                    )
    ...
    def update_forward_cost_of_children(
        self, n: BaseNode, start_nodes_to_update: set[int]
    ) -> set[int]:
        """
    ...
    def update_forward_cost(self, node: BaseNode) -> None:
        ...
        if len(node.forward.children) > 0:
            start_nodes_to_update = self.update_forward_cost_of_children(
                node, start_nodes_to_update
            )
    ...
    def compute_transition_lb_cost_to_come(self) -> None:
        ...
        while len(queue) > 0:
            ...
            if mode not in transition_nodes:
                transition_nodes[mode] = [
                    self.nodes[id] for id in self.transition_node_ids[mode]
                ]
    ...
    def compute_transition_lb_effort_to_come(self) -> None:
        ...
        while len(queue) > 0:
            ...
            edge_efforts = (
                self.batch_dist_fun(
                    node.state.q, self.transition_node_array_cache[mode], c="max"
                )
                / self.collision_resolution
            )
        ...
        for mode in self.node_ids:
            for id in self.node_ids[mode]:
                ...
                if self.including_effort:
                    ...
                    n.lb_effort_to_come = np.minimum.reduce(
                        reverse_lb_array + efforts_to_transitions
                    )

                # potential_costs = reverse_lb_array + costs_to_transitions
                n.lb_cost_to_come = np.minimum.reduce(
                    reverse_lb_array + costs_to_transitions
                )
                processed += 1
    ...
    def update_forward_queue_keys(
        self, type: str, node_ids: Optional[Set[BaseNode]] = None
    ) -> None:
        """
...
class InformedSampling(BaseInformedSampling):
    def lb_cost_from_start(self, state: State, g, lb_attribute_name="lb_cost_to_come"):
        if state.mode not in g.reverse_transition_node_array_cache:
            ...
            g.reverse_transition_node_array_cache[state.mode] = np.empty(
                (len(ids), g.dim), dtype=np.float64
            )
            for i, id in enumerate(ids):
                g.reverse_transition_node_array_cache[state.mode][i] = g.nodes[
                    id
                ].state.q.q

        if state.mode not in g.reverse_transition_node_lb_cache:
            ids = g.reverse_transition_node_ids[state.mode]
            g.reverse_transition_node_lb_cache[state.mode] = np.empty(
                len(ids), dtype=np.float64
            )
            for i, id in enumerate(ids):
                g.reverse_transition_node_lb_cache[state.mode][i] = g.nodes[
                    id
                ].lb_cost_to_come
    ...
    def lb_cost_from_goal(self, state: State, g, lb_attribute_name="lb_cost_to_go"):
        ...
        if state.mode not in g.transition_node_array_cache:
            ...
            g.transition_node_array_cache[state.mode] = np.empty(
                (len(ids), g.dim), dtype=np.float64
            )
            for i, id in enumerate(ids):
        ...
        if state.mode not in g.transition_node_lb_cache:
            ids = g.transition_node_ids[state.mode]
            g.transition_node_lb_cache[state.mode] = np.empty(
                len(ids), dtype=np.float64
            )
            for i, id in enumerate(ids):
...
class BaseLongHorizon:
    ...
    def init_long_horizon(
        ...
        if self.rewire:
            ...
            or end_idx >= len(tot_mode_sequence) - self.horizon_length - 1
            or (end_idx + 1 >= len(tot_mode_sequence) - 2 and self.horizon_length > 1)
        ):
            self.rewire = True
            end_idx = len(tot_mode_sequence) - 2
    ...
    def update_virtual_goal_nodes(
        self, g: "BaseGraph", tot_mode_sequence: List["Mode"]
    ) -> None:
        """
        ...
        if self.terminal_mode == tot_mode_sequence[-1]:
            g.virtual_goal_nodes = [
                g.nodes[id] for id in g.transition_node_ids[self.terminal_mode]
            ]
...
class BaseITstar(BasePlanner):
    ...
    def sample_valid_uniform_batch(
        self, batch_size: int, cost: float | None
    ...
    def create_virtual_root(self) -> None:
        ...
        for transition in transition_nodes:
            ...
            if node.forward.parent is not None:
                potential_sorted_reached_modes = [
                    m for m in self.sorted_reached_modes if m not in self.expanded_modes
                ]
                if self.get_virtual_root(
                    node.transition_neighbors, potential_sorted_reached_modes[0]
                ):
        ...
        if prev_mode is None:
            ...
            for transition in self.g.transition_node_ids[prev_mode]:
                node = self.g.nodes[transition]
                if node.forward.parent is not None:
                    if not self.get_virtual_root(
                        node.transition_neighbors, self.last_expanded_mode
                    ):
                        return
                    break
    ...
    def get_virtual_root(
        self, transition_neighbors: List[BaseNode], next_mode: Mode
    ) -> bool:
        """
        ...
        for transition in transition_neighbors:
            if next_mode == transition.state.mode:
                ...
                if (
                    index != 0
                    and transition.forward.parent.state.mode
                    != self.sorted_reached_modes[index - 1]
                ):
                    return False
    ...
    def sample_valid_uniform_transitions(
        self, transistion_batch_size: int, cost: float | None
    ) -> None:
        """
        ...
        if (
            ...
            and self.current_best_cost is None
            and len(self.g.goal_nodes) > 0
        ):
        ...
        update = (
            ...
            and (self.long_horizon.init or self.long_horizon.reached_terminal_mode)
        )
        ...
        while (
            ...
            and failed_attempts < 6 * transistion_batch_size
        ):
            # sample mode
            ...
            if reached_terminal_mode:
                ...
                possible_next_task_combinations = (
                    self.env.get_valid_next_task_combinations(mode)
                )
            ...
            if self.env.is_collision_free(q, mode):
                if self.env.is_terminal_mode(mode):
                    ...
                    if reached_terminal_mode:
                        if mode not in self.init_next_modes:
                            ...
                            next_modes = self.mode_validation.get_valid_modes(
                                mode, tuple(next_modes)
                            )
                ...
                if (
                    len(list(chain.from_iterable(self.g.transition_node_ids.values())))
                    > transitions
                ):
                    transitions += 1
                    if self.empty_transition_nodes is not None:
    ...
    def get_init_mode_sequence(self, mode: Mode) -> List[Mode]:
        ...
        if self.current_best_cost is None and len(self.g.goal_nodes) > 0:
            ...
            if self.config.apply_long_horizon and self.long_horizon.init:
                self.long_horizon.init_long_horizon(
                    self.g, self.current_best_path_nodes, self.sorted_reached_modes
                )
    ...
    def create_mode_init_sequence(self, mode: Mode) -> None:
        ...
        if (
            self.dummy_start_mode
            and self.init_search_modes[0] == self.g.root.state.mode
        ):
            self.init_search_modes = self.init_search_modes[1:]
    ...
    def add_sample_batch(self) -> None:
        ...
        while True:
            uniform_batch_size = (
                self.current_best_cost is not None
                or not self.first_search
                or self.config.apply_long_horizon
                and self.first_search
                and not self.long_horizon.init
            )
            ...
            if (
                ...
                self.long_horizon.update_virtual_goal_nodes(
                    self.g, self.sorted_reached_modes
                )
    ...
    def sample_manifold(self) -> None:
        ...
        while True:
            ...
            if (
                not self.config.apply_long_horizon
                and self.current_best_cost is None
                and not self.first_search
            ):
                self.remove_nodes_in_graph_before_init_sol()
            if (
                self.config.apply_long_horizon
                and not self.long_horizon.new_section
                and not self.long_horizon.reached_terminal_mode
            ):
                self.remove_nodes_in_graph_before_init_sol()
            if (
                self.config.apply_long_horizon
                and self.long_horizon.new_section
                and not self.long_horizon.init
            ):
                self.long_horizon.init_long_horizon(
                    self.g, self.current_best_path_nodes, self.sorted_reached_modes
                ...
                if not self.long_horizon.reached_terminal_mode:
                    self.first_search = True
            if (
                self.current_best_path is not None
    ...
    def prune_set_of_expanded_nodes(self) -> Tuple[List[int], List[int]]:
        ...
        while len(stack) > 0:
            ...
            if (
                not self.config.remove_based_on_modes
                or key not in self.start_transition_arrays
            ):
                if key not in focal_points:
                    focal_points[key] = np.array(
                        [
                            self.g.root.state.q.state(),
                            self.current_best_path[-1].q.state(),
                        ],
                        dtype=np.float64,
                    )
                ...
                if key not in focal_points:
                    focal_points[key] = np.array(
                        [
                            ...
                            self.end_transition_arrays[key],
                        ],
                        dtype=np.float64,
                    )
            ...
            if node in self.current_best_path_nodes:
                ...
                flag = (
                    sum(self.env.batch_config_cost(node.state.q, focal_points[key]))
                    > inter_costs[key]
                )
                if not flag:
                ...
                flag = (
                    sum(
                        self.env.batch_config_cost(
                            node.state.q, focal_points_transition[key]
                        )
                    )
                    > inter_costs[key]
                )
        ...
        goal_mask = np.array(
            [item in children_to_be_removed for item in self.g.goal_node_ids]
        )
    ...
    def remove_nodes_in_graph(self) -> None:
        ...
        for mode in list(
            ...
            if (
                not self.config.remove_based_on_modes
                or mode not in self.start_transition_arrays
            ):
                focal_points = np.array(
                ...
                focal_points = np.array(
                    [
                        ...
                        self.end_transition_arrays[mode],
                    ],
                    dtype=np.float64,
                )
            ...
            self.g.node_ids[mode] = [
                ...
                or (self.g.virtual_root is not None and id == self.g.virtual_root.id)
                or id in vertices_to_keep
                or (
                    id not in vertices_to_be_removed
                    and sum(
                        self.env.batch_config_cost(
                            self.g.nodes[id].state.q, focal_points
                        )
                    )
                    <= cost
                )
        ...
        for mode in list(self.g.transition_node_ids.keys()):
            if (
                self.env.is_terminal_mode(mode)
                or mode == self.long_horizon.terminal_mode
            ):
                continue
            if (
                not self.config.remove_based_on_modes
        ...
        valid_mask = np.array(
            [bool(node.transition_neighbors) for node in transition_nodes]
        )
    ...
    def remove_nodes_in_graph_before_init_sol(self) -> None:
        ...
        Returns:
            None
        """
        relevant_expanded_modes = [
            mode
            for mode in list(self.g.tree.keys())
            if mode in self.sorted_reached_modes
        ]
        if relevant_expanded_modes != []:
            if (
                self.dummy_start_mode
                and relevant_expanded_modes[0] == self.g.root.state.mode
            ):
                relevant_expanded_modes = relevant_expanded_modes[1:]
            self.expanded_modes.update(relevant_expanded_modes)
            self.last_expanded_mode = max(
                relevant_expanded_modes, key=lambda obj: obj.id
            )
        else:
            if self.config.apply_long_horizon:
                return
            ...
            if self.g.virtual_root is not None:
                virtual_root_mode = self.g.virtual_root.transition_neighbors[
                    0
                ].state.mode
                self.sorted_reached_modes.insert(0, virtual_root_mode)
                self.g.reverse_transition_node_ids[
                    self.g.virtual_root.state.mode
                ].remove(self.g.virtual_root.id)
                self.g.transition_node_ids[virtual_root_mode].remove(
                    self.g.virtual_root.transition_neighbors[0].id
                )
        ...
        for mode in list(
            ...
            if (
                self.config.apply_long_horizon
                and mode not in self.long_horizon.mode_sequence
            ):
                continue
            ...
            self.g.node_ids[mode] = [
                ...
                for id in self.g.node_ids[mode]
                if id == self.g.root.id
                or id in BaseTree.all_vertices
                or (
                    not self.config.with_mode_validation
                    and not np.isinf(self.g.nodes[id].cost)
                )
            ]
            num_pts_for_removal += original_count - len(self.g.node_ids[mode])
        self.g.reverse_transition_node_ids = {}
    ...
    def process_valid_path(
        ...
        if (
            force_update
            or self.current_best_cost is None
            or new_path_cost < self.current_best_cost
        ):
            self.alpha = 1.0
            ...
            print()
            print(
                f"found cost: {new_path_cost} at time {time.time() - self.start_time}"
            )
            ...
            if self.config.try_shortcutting and with_shortcutting:
                ...
                if shortcut_path_cost < self.current_best_cost:
                    ...
                    lh = (
                        self.config.apply_long_horizon
                        and not self.env.is_terminal_mode(shortcut_path[-1].mode)
                    )
                    self.g.add_path_states(
                        interpolated_path,
                        self.approximate_space_extent,
                        rewire=rewire,
                        lh=lh,
                    )
                    self.current_best_path_nodes = self.generate_path(
                        force_generation=True
                    )
                    self.current_best_path = [
                        node.state for node in self.current_best_path_nodes
                    ]
                    ...
                    if not (
                        ...
                        self.add_reverse_connectivity_to_path(
                            self.current_best_path_nodes, with_queue_update
                        )

                    self.current_best_cost = path_cost(
                        self.current_best_path, self.env.batch_config_cost
                    )
                    self.update_results_tracking(
                        self.current_best_cost, self.current_best_path
                    )
    ...
    def update_removal_conditions(self) -> None:
        ...
        (
            self.start_transition_arrays,
            self.end_transition_arrays,
            self.intermediate_mode_costs,
        ) = {}, {}, {}
        self.start_transition_arrays[self.current_best_path_nodes[0].state.mode] = (
            self.g.root.state.q.state()
        )
        ...
        self.end_transition_arrays[modes[-1]] = self.current_best_path_nodes[
            -1
        ].state.q.state()
        self.intermediate_mode_costs[modes[-1]] = (
            self.current_best_path_nodes[-1].cost - start_cost
        )
        print("Modes of new path")
        print([m.task_ids for m in modes])

    def add_reverse_connectivity_to_path(
        self, path: List[BaseNode], with_update: bool
    ) -> None:
        """
        ...
        for node in reversed(path[1:-1]):
            ...
            self.g.update_connectivity(
                parent, node, edge_cost, parent.lb_cost_to_go + edge_cost, "reverse"
            )
    ...
    def get_lb_goal_node_and_cost(self) -> BaseNode:
        ...
        if (
            ...
            min_id = np.argmin(
                self.operation.costs[self.g.virtual_goal_node_ids], axis=0
            )
    ...
    def expand_node_forward(
        ...
        neighbors = self.g.get_neighbors(
            node, space_extent=self.approximate_space_extent, first_search=first_search
        )
        if neighbors.size == 0:
            return
        ...
        for id, edge_cost, edge_effort in zip(neighbors, edge_costs, edge_efforts):
            ...
            if (
                self.config.apply_long_horizon
                and n.state.mode not in self.long_horizon.mode_sequence
                and node.id != self.g.root.id
            ):
                continue
    ...
    def save_tree_data(self, nodes: Tuple[set]) -> None:
        ...
        data = {}
        data["all_nodes"] = [
            self.g.nodes[id].state.q.state()
            for id in list(chain.from_iterable(self.g.node_ids.values()))
        ]
        data["all_transition_nodes"] = [
            self.g.nodes[id].state.q.state()
            for id in list(chain.from_iterable(self.g.transition_node_ids.values()))
        ]
        data["all_nodes_mode"] = [
            self.g.nodes[id].state.mode.task_ids
            for id in list(chain.from_iterable(self.g.node_ids.values()))
        ]
