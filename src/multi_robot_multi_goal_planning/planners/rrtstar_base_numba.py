...
class BaseRRTstar(ABC):
    def __init__(self, env, config: ConfigManager):
        self.env = env
        self.config = config
        self.eta = np.sqrt(sum(self.env.robot_dims.values())) #TODO how much bigger should one make this?
        self.operation = Operation()
    ...
    def get_lebesgue_measure_of_free_configuration_space(self, num_samples=10000):
        ...
        for i in range(num_samples):
            ...
            if self.env.is_collision_free_without_mode(q):
                free_samples += 1
        # self.SaveData(None, time.time()-self.start, ellipse=q_ellipse)
    ...
    def SampleNodeManifold(self, mode:Mode) -> Configuration:
        if np.random.uniform(0, 1) <= self.config.p_goal:
            ...
            if self.config.informed_sampling and self.operation.init_sol: 
                if self.config.informed_sampling_version == 0 and np.random.uniform(0, 1) > self.config.p_uniform or self.config.informed_sampling_version == 5 and np.random.uniform(0, 1) > self.config.p_uniform:
                    #uniform sampling
    ...
    def UpdateDiscretizedCost(self, path , cost, idx):
        ...
        batch_cost = batch_config_cost(path_a, path_b, metric = self.config.cost_metric, reduction=self.config.cost_reduction)
        ...
        for i in range(range1):
            for j in range(range2):
                ...
                if edge_cost[-1] < discretized_costs[idx2] and self.env.is_path_collision_free(edge, resolution=0.001, tolerance=0.001): #need to make path_collision_free
                    ...
                    # a+=1
                    # if a >= 86:
                    #     self.env.is_path_collision_free(edge)
                    #     print("")
                    if not deterministic:
    ...
    def EdgeInterpolation(self, path, cost, indices, dim, version, r = None, robots =None):
        ...
        for i in range(len(path)):
            ...
            batch_cost = batch_config_cost(edge_a, edge_b, metric = self.config.cost_metric, reduction=self.config.cost_reduction)
