...
from multi_robot_multi_goal_planning.problems import get_env_by_name
from analysis.postprocessing import final_path_animation
...
def setup_planner(
    ...
    if planner_config["type"] == "rrtstar":
        def planner(env):
    ...
    elif planner_config["type"] == "birrtstar_par":
        def planner(env):
            options = planner_config["options"]
            return ParallelizedBidirectionalRRTstar(
                    env, 
                    ptc=RuntimeTerminationCondition(runtime),
                    # general_goal_sampling=options["general_goal_sampling"],
                    informed_sampling=options["informed_sampling"],
...
def main():
    ...
    for planner in config["planners"]:
        for i, run in enumerate(all_experiment_data[planner['name']]):
            path = [state.q.state() for state in run['paths'][-1]]
            if len(path[0])/(len(env.robots)) <= 3: # Only applicable up to 2D env with orientation
                output = os.path.join(experiment_folder, planner['name'])
                output_html = os.path.join(output, f'{i}/final_path_animation_3d.html')
                env_path = os.path.join(os.path.expanduser("~"), f'env/{config["environment"]}')
                save_env_as_mesh(env, env_path)
                final_path_animation(env, env_path, path, output_html)
